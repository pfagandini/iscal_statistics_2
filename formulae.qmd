...
\centering
\small
\begin{tabular}{>{\centering\arraybackslash}p{0.7cm} p{3.8cm} p{4.2cm} p{5.8cm}}
\hline
\textbf{\#} & \textbf{Population} & \textbf{Distribution of $\bar{X}$} & \textbf{$(1-\alpha)\times 100\%$ Confidence Interval} \\
\hline
1 &
  $X\sim N(\mu,\sigma)$; $\sigma^2$ \textbf{known} &
...
2 &
  $X\sim N(\mu,\sigma)$; $\sigma^2$ \textbf{unknown} &
  $T = \dfrac{\bar{X}-\mu}{S'/\sqrt{n}}\sim t_{(n-1)}$ &
  $\left(\bar{x} - t_{\alpha/2}\dfrac{s'}{\sqrt{n}},\; \bar{x} + t_{\alpha/2}\dfrac{s'}{\sqrt{n}}\right)$ \\
2b &
  $X\sim N(\mu,\sigma)$; $\sigma^2$ \textbf{unknown}, $n \geq 30$ &
  $Z = \dfrac{\bar{X}-\mu}{S'/\sqrt{n}}\;\dot{\sim}\; N(0,1)$ &
  $\approx\left(\bar{x} - z_{\alpha/2}\dfrac{s'}{\sqrt{n}},\; \bar{x} + z_{\alpha/2}\dfrac{s'}{\sqrt{n}}\right)$ \\
3a &
  $X\sim{?}$; $\sigma^2$ \textbf{known}, $n \geq 30$ &
  $Z = \dfrac{\bar{X}-\mu}{\sigma/\sqrt{n}}\;\dot{\sim}\; N(0,1)$ &
  $\approx\left(\bar{x} - z_{\alpha/2}\dfrac{\sigma}{\sqrt{n}},\; \bar{x} + z_{\alpha/2}\dfrac{\sigma}{\sqrt{n}}\right)$  \\
3b &
  $X\sim{?}$; $\sigma^2$ \textbf{unknown}, $n \geq 30$ &
  $Z = \dfrac{\bar{X}-\mu}{S'/\sqrt{n}}\;\dot{\sim}\; N(0,1)$ &
  $\approx\left(\bar{x} - z_{\alpha/2}\dfrac{s'}{\sqrt{n}},\; \bar{x} + z_{\alpha/2}\dfrac{s'}{\sqrt{n}}\right)$ \\
\hline
\end{tabular}
...
\hline
\textbf{Population} & \textbf{Distribution of $\hat{p} = X/n$} & \textbf{$(1-\alpha)\times 100\%$ Confidence Interval} \\
\hline
$X\sim \mathrm{Binomial}(n,p)$, $n\geq 30$ &
  $\dfrac{\hat{p}-p}{\sqrt{\hat{p}(1-\hat{p})/n}}\;\dot{\sim}\; N(0,1)$ &
  $\left(\hat{p} - z_{\alpha/2}\sqrt{\dfrac{\hat{p}(1-\hat{p})}{n}},\; \hat{p} + z_{\alpha/2}\sqrt{\dfrac{\hat{p}(1-\hat{p})}{n}}\right)$ \textit{(approx.)} \\
\hline
\end{tabular}
\end{table}
\renewcommand{\arraystretch}{1}
...
\hline
\textbf{Population} & \textbf{Distribution of $S'^{\,2}$} & \textbf{$(1-\alpha)\times 100\%$ Confidence Interval} \\
\hline
$X\sim N(\mu,\sigma)$ &
...
\begin{table}[h]
\centering
\begin{tabular}{p{6.0cm} >{\centering\arraybackslash}p{2.8cm} p{4.8cm}}
\hline
\textbf{Hypotheses} & \textbf{Test Statistic (TS)} & \textbf{Critical Region} \\
\hline
$H_0: \theta = \theta_0 \;\text{vs}\; H_1: \theta \neq \theta_0$ &
  $\mathrm{TS}_0$ &
  $(-\infty,\, q_{\alpha/2}) \cup (q_{1-\alpha/2},\, +\infty)$ \\
